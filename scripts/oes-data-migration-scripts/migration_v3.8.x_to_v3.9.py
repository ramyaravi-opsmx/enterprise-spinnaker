...
def update_usergroup_permission_data(old_user_group_id, new_user_group_id):
    try:
        ...
        if result > 0:
            cur.execute(
                "update user_group_permission set group_id = " + str(new_user_group_id) + " where group_id =" + str(
                    old_user_group_id))
    except Exception as e:
...
def update_usergroup_name_tolower(user_group_id, user_group_name):
    try:
        ...
        result = cur.fetchone()[0]
        if result == 0:
            cur.execute('UPDATE user_group SET name =' + str(user_group_name) + ' WHERE id =' + str(user_group_id))
    except Exception as e:
...
def alter_approval_gate_parameter():
    try:
        ...
        cur.execute(
            "ALTER TABLE approval_gate_parameter ADD COLUMN IF NOT EXISTS param_group_id INT NOT NULL DEFAULT 0")
    except Exception as e:
...
def fetch_distinct_gate_instances():
    try:
        cur = visibility_conn.cursor()
        cur.execute(
            "select distinct(approval_gate_instance_id) from approval_gate_parameter ORDER BY approval_gate_instance_id")
        return cur.fetchall()
    except Exception as e:
...
def fetch_distinct_connectors(gate_instance_id):
    try:
        cur = visibility_conn.cursor()
        cur.execute(
            "select distinct(connector_type) from approval_gate_parameter where approval_gate_instance_id = " + str(
                gate_instance_id))
        return cur.fetchall()
    except Exception as e:
...
def update_param_group_id(param_group_id, connector_type, approval_gate_instance_id):
    try:
        cur = visibility_conn.cursor()
        cur.execute("update approval_gate_parameter set param_group_id = " + str(
            param_group_id) + " where connector_type = '" + str(
            connector_type) + "' AND approval_gate_instance_id = " + str(approval_gate_instance_id))
    except Exception as e:
    ...
    try:
        ...
        if len(pipe_json) > 0:
            ...
            if stages is not None and len(stages) > 0:
                ...
                cur.execute(
                    "update pipeline set pipeline_json = '" + str(json.dumps(pipe_json)) + "' where id=" + str(id))
...
def fetch_configured_tool_connectors(approval_gate_id):
    try:
        url = oes_visibility_url + "/visibilityservice/v1/approvalGates/" + str(
            approval_gate_id) + "/configuredtoolConnectors"
...
def alter_cloud_configuration():
    try:
        ...
        cur.execute(
            "ALTER TABLE cloud_configuration ALTER COLUMN bakery_secret_key TYPE varchar(10000) USING bakery_secret_key::varchar")
        cur.execute(
            "ALTER TABLE cloud_configuration ALTER COLUMN secret_key TYPE varchar(10000) USING secret_key::varchar")
    except Exception as e:
...
if __name__ == '__main__':
    ...
    if n != 10:
        print(
            'Please pass valid 9 arguments <platform-db-name> <platform-db-host> <visibility-db-name> <visibility-db-host> <sapor-db-name> <sapor-db-host> <oes-visibility-url> <db-port> <oes-admin-user>')
